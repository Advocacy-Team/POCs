CLASS zadt_sma_cl_file_into_it DEFINITION
  PUBLIC
  FINAL
  CREATE PUBLIC .
  ...
  PRIVATE SECTION.
    ...
    DATA: mv_web           TYPE string VALUE 'https://my400036-api.lab.s4hana.cloud.sap:443/sap/opu/odata/sap/',
...
CLASS zadt_sma_cl_file_into_it IMPLEMENTATION.
...
*          REPLACE FIRST OCCURRENCE OF REGEX '\C+,"ServiceEntrySheet"' IN lv_purch_orders WITH '{"ServiceEntrySheet"'.
*          REPLACE FIRST OCCURRENCE OF REGEX ',"to_\C+' IN lv_purch_orders WITH '}'.


*          /ui2/cl_json=>generate(
*            EXPORTING
*              json          = lv_purch_orders
**              pretty_name   =
**              name_mappings =
*            RECEIVING
*              rr_data       = DATA(lt_json)
*          ).
*
*
*
...
*                                      i_value = |attachment; filename="example.json"| ).
          ...
          response->set_text( lv_purch_orders ).
          "|You are not authorized for creating service entry sheet| ).
...
*    COMMIT ENTITIES END.
  ...
  METHOD get_purch_orders.
    ...
    lo_req->set_header_fields( VALUE #(
     ...
     ( name = 'APIKey' value = 'fgTDyAQAB2oPextZunYysnDo4aKhLXGF' )
     ) ).

    lo_req->set_uri_path( i_uri_path = mv_web && 'API_SERVICE_ENTRY_SHEET_SRV/A_ServiceEntrySheet?$top=1' ).
...
*    .  "  SAP_CONVERT_TO_CSV_FORMAT
    ...
    TRY.
        ...
        DATA(lo_http_destination) =
             cl_http_destination_provider=>create_by_url( 'https://my400036-api.lab.s4hana.cloud.sap:443/sap/opu/odata/sap/API_SERVICE_ENTRY_SHEET_SRV/A_ServiceEntrySheet?$top=50' ).
...
*        TRY.
*            lo_web_http_request->set_csrf_token( ).
...
*        DATA(lr_get_csrf) = lo_web_http_request->get_header_field( EXPORTING i_name = 'X-CSRF-Token' ).
*        lo_web_http_request->set_header_field( EXPORTING i_name = 'X-CSRF-Token' i_value = 'Fetch' ). "lr_get_csrf ).
  ...
  METHOD post_entry_sheet.
    TRY.
        "create http destination by url; API endpoint for API sandbox
        DATA(lo_http_destination) =
             cl_http_destination_provider=>create_by_url( 'https://my400036-api.lab.s4hana.cloud.sap:443/sap/opu/odata/sap/API_SERVICE_ENTRY_SHEET_SRV/A_ServiceEntrySheet' ).
...
*        lo_request->set_query( EXPORTING query = '$filter=Material eq ''4900000000'',$filter=Plant eq ''1710'' ').
        ...
        lo_web_http_request->set_text('{ "PurchaseOrder" : "4500004461", "ServiceEntrySheetName" : "SES_TEST" }').
        "set request method and execute request
