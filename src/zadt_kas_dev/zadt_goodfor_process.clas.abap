CLASS zadt_goodfor_process DEFINITION
  PUBLIC
  FINAL
  CREATE PUBLIC .
...
CLASS zadt_goodfor_process IMPLEMENTATION.
...
*  Get plants and materials relevant for the goodfor process
    SELECT * FROM ZADT_I_Good_For_M INTO TABLE @lt_yggood_for.

*  Read records from the WaitingFor table that have not been processed yet
    SELECT * FROM ZADT_I_Waiting_For_M INTO TABLE @it_up_waiting_for.

*  Select Inspection Load data with COMPLETED status( NOT EXACTLY!!!! )  ZADT_I_InspectionLot_M
    SELECT * FROM  I_InspectionLot  AS a
      INNER JOIN I_InspectionResult AS b
      ON a~InspectionLot = b~InspectionLot
      WHERE b~InspectionResultStatus = '5'
      INTO TABLE @DATA(lt_insplot).

    IF sy-subrc = 0.
*  Collect list of Inspection Lots for processing
      LOOP AT lt_insplot ASSIGNING FIELD-SYMBOL(<fs_insp>).

        it_waiting_for = VALUE #( ( plant    = <fs_insp>-a-Plant
                                    material = <fs_insp>-a-Material
                                    batch    = <fs_insp>-a-Batch
                                    insplot  = <fs_insp>-a-InspectionLot
...
*  Update ZADT_WAITING_FOR table using EML
...
*  Read planned orders for each plant and potential goodfor material
      SELECT PlannedOrder, Material, MRPPlant
        FROM I_PlannedOrder FOR ALL ENTRIES IN @lt_insplot
       WHERE material = @lt_insplot-a-material
         AND mrpplant = @lt_insplot-a-plant
      ...
      IF sy-subrc = 0.
          ...
          TRY.
              ...
              DATA(lv_request) = |$filter=Material eq '{ 'ZTEST_RAW_MAT' }',$filter=Plant eq '{ '1010' }'|.

              lo_request->set_query( EXPORTING query = lv_request ).
      ...
      LOOP AT lt_yggood_for ASSIGNING FIELD-SYMBOL(<lfs_insp>)."lt_good_for
        ...
        TRY.
            DATA(lo_dest1) = cl_http_destination_provider=>create_by_url(
                 i_url     = 'https://my400036-api.s4hana.ondemand.com/sap/opu/odata/sap/API_MATERIAL_DOCUMENT_SRV/A_MaterialDocumentHeader'
              ).
...
*// Update Batch Characteristics
      LOOP AT lt_yggood_for  ASSIGNING FIELD-SYMBOL(<lfs_goodfor>)."lt_good_for
        ...
        TRY.
            ...
            i_url    = 'https://my400036-api.s4hana.ondemand.com/sap/opu/odata/sap/API_BATCH_SRV/Batch(Material="{<lfs_goodfor>-good_for_pq}", BatchIdentifyingPlant="{<lfs_goodfor>-plant}", Batch="0000000001")'
              ).
          CATCH cx_http_dest_provider_error.
            "handle exception
        ENDTRY.
